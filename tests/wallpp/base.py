...
def test_xygrid(new=0,pp_type='p4'):
    if new:
        pattern,lat_vec = np.load(datpath+'cygne2_100_%s.npy'%pp_type,allow_pickle=True)
...
def test_wiki_wallpp():
    for pp_type in cg.pp_types[1:]:
        ...
        pp.plot(opts='aug',nh=2,nk=2,title=pp_type,pOpt='tXe',
            name=figpath+'wiki_%s.png' %pp_type,opt='sc')
...
def test_swan_wallpp(params = {'a':1.5,'b':1,'alpha' : 80}):
    ...
    im = plt.imread(figpath+'cygne2_100.png')
    z = np.flipud(im[:,:,2])
    nx,ny = z.shape
