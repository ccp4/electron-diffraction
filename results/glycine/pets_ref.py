...
# i(integrate on frame manually) c(Fo vs Fc), o(check Fo=sqrt(Io))
opts=''
...
if 'I' in opts:
    ...
    refl = pets.hkl.index
    sumI = [pets.rpl.loc[pets.rpl.hkl==r,'I'].mean() for r in refl]
...
## checked that Fo=sqrt(Io)
if 'o' in opts:
    idx = [h for h in df.index if h in pets.HKL_dyn.index]
    Fo  = np.sqrt(pets.HKL_dyn.loc[idx,'I'])
    plts=[[df.loc[idx,'Fo'],Fo,'rs']]
    dsp.stddisp(plts,labs=['$F_o$','$F_c$'])
